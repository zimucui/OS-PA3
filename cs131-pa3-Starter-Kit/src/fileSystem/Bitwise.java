...
public class Bitwise {
    ...
    public static boolean isset(int i, byte bytes[]) {//FIXME!!!
    	int module = i % 8;
    	int integer = i / 8;
    	int locationInArray = bytes.length - 1 - integer;
    	if (bytes[locationInArray] == (byte) (bytes[locationInArray] | bitmasks[module])) {
    		return true;
    	} else {
    		return false;
    	}
    ...
    public static void set(int i, byte bytes[]) {//FIXME!!!
    	...
    	int integer = i / 8;
    	int locationInArray = bytes.length - 1 - integer;
    	bytes[locationInArray] = (byte) (bytes[locationInArray] | bitmasks[module]);
    }
    ...
    public static boolean clear(int i, byte bytes[]) {//FIXME!!!
    	...
    	int integer = i / 8;
    	int locationInArray = bytes.length - 1 - integer;
    	byte test = (byte) (bytes[locationInArray] | bitmasks[module]);
    	if (test != bytes[locationInArray]) {
    		return false;
    	} else {
    		byte flip = (byte) ~ bytes[locationInArray];
    		flip = (byte) (flip | bitmasks[module]);
    		bytes[locationInArray] = (byte) ~ flip;
    		return true;
    	}
    }
