...
public class MyFileSystem implements FileSystem {
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
        ...
        int singlyIndiMax = 10 + IndirectBlock.COUNT;
        int ptrNumber1 = blockNum - 10;         // for singly indirection block use
        int doublyIndiMax = 10 + IndirectBlock.COUNT + IndirectBlock.COUNT*IndirectBlock.COUNT;
        int ptrNumber2 = (blockNum - singlyIndiMax) / IndirectBlock.COUNT;      //for singly indirection block use
        int ptrNumber3 = (blockNum - singlyIndiMax) % IndirectBlock.COUNT;		//for doubly indirection block use
        int triplyIndiMax = 10 + IndirectBlock.COUNT + IndirectBlock.COUNT*IndirectBlock.COUNT + IndirectBlock.COUNT*IndirectBlock.COUNT*IndirectBlock.COUNT;
        int ptrNumber4 = (blockNum - doublyIndiMax) / (IndirectBlock.COUNT * IndirectBlock.COUNT);    //for singly indirection block use
        int ptrNumber5 = (blockNum - doublyIndiMax) / IndirectBlock.COUNT % IndirectBlock.COUNT;       //for doubly indirection block use
        int ptrNumber6 = (blockNum - doublyIndiMax) % IndirectBlock.COUNT;       //for triply indirection block use
        // build three indirection blocks at the very beginning
        IndirectBlock singlyIndiBlock = new IndirectBlock(); 
        IndirectBlock doublyIndiBlock = new IndirectBlock();
        IndirectBlock triplyIndiBlock = new IndirectBlock();
        
        if(blockNum <= 9) {              //when we don't need indirection
        ...
        } else if (blockNum >= 10 && blockNum < singlyIndiMax) {  //now we use singly indirection
        	boolean fresh = inode.ptr[10] == 0;
        	if (!fresh) {    //when inode.ptr[blockNum] is not 0
        		disk.read(inode.ptr[10], singlyIndiBlock);           //read from existing one
        		fresh = singlyIndiBlock.ptr[ptrNumber1] == 0;
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if((singlyIndiBlock.ptr[ptrNumber1] = freeMap.find()) == 0) {     //check if there is space
                        return null;
                    }
        		}
        		disk.write(inode.ptr[10], singlyIndiBlock);        //save change
        	} else {      // if fresh
        		if(mode == MODE.r) {
                    return DirectBlock.hole;
        	...
        	return new DirectBlock(disk, singlyIndiBlock.ptr[ptrNumber1], blockOff, fresh);
     	
        } else if (blockNum >= singlyIndiMax && blockNum < doublyIndiMax) {    //double indirection
        	boolean fresh = inode.ptr[11] == 0;
        	if (!fresh) {
        		disk.read(inode.ptr[11], singlyIndiBlock);   //read from existing one
        		fresh = singlyIndiBlock.ptr[ptrNumber2] == 0;
        		if (!fresh) {
            		disk.read(singlyIndiBlock.ptr[ptrNumber2], doublyIndiBlock);
        		} else {  //when it is fresh
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if((singlyIndiBlock.ptr[ptrNumber2] = freeMap.find()) == 0) {     //check if there is space
                        return null;
                    } 
        		}
        		disk.write(inode.ptr[11], singlyIndiBlock);        //save change
        		
        		fresh = doublyIndiBlock.ptr[ptrNumber3] == 0;
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if((doublyIndiBlock.ptr[ptrNumber3] = freeMap.find()) == 0) {     //check if there is space
                        return null;
                    }
        		}
        		disk.write(singlyIndiBlock.ptr[ptrNumber2], doublyIndiBlock);        //save change
        		...
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if ((singlyIndiBlock.ptr[ptrNumber2] = freeMap.find()) == 0) {
                        return null;
                    } 
        		}
        		disk.write(inode.ptr[11], singlyIndiBlock);       //save change
        		...
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if ((doublyIndiBlock.ptr[ptrNumber3] = freeMap.find()) == 0) {
                        return null;
                    }
        		}
        		disk.write(singlyIndiBlock.ptr[ptrNumber2], doublyIndiBlock);      //save change
    		...
    		return new DirectBlock(disk, doublyIndiBlock.ptr[ptrNumber3], blockOff, fresh);
  
        } else if (blockNum >= doublyIndiMax && blockNum < triplyIndiMax) {      //triple indirection
        	boolean fresh = inode.ptr[12] == 0;
        	if (!fresh) {
        		disk.read(inode.ptr[12], singlyIndiBlock);     //read from existing one
        		
        		fresh = singlyIndiBlock.ptr[ptrNumber4] == 0;
        		
        		if (!fresh) {
        			disk.read(singlyIndiBlock.ptr[ptrNumber4], doublyIndiBlock);     //read from existing one
        		} else {  //when it is fresh
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if((singlyIndiBlock.ptr[ptrNumber4] = freeMap.find()) == 0) {     //check if there is space
                        return null;
                    } 
        		}
        		disk.write(inode.ptr[12], singlyIndiBlock);    //save change
        		
        		fresh = doublyIndiBlock.ptr[ptrNumber5] == 0;
        		
        		if (!fresh) {
        			disk.read(doublyIndiBlock.ptr[ptrNumber5], triplyIndiBlock);
        		} else {  //when it is fresh
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if((doublyIndiBlock.ptr[ptrNumber5] = freeMap.find()) == 0) {     //check if there is space
                        return null;
                    } 
        		}
        		disk.write(singlyIndiBlock.ptr[ptrNumber4], doublyIndiBlock);      //save change
        		
        		fresh = triplyIndiBlock.ptr[ptrNumber6] == 0;
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if ((triplyIndiBlock.ptr[ptrNumber6] = freeMap.find()) == 0) {
                        return null;
                    }
        		}
        		disk.write(doublyIndiBlock.ptr[ptrNumber5], triplyIndiBlock);        //save change
        		...
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if ((singlyIndiBlock.ptr[ptrNumber4] = freeMap.find()) == 0) {
                        return null;
                    } 
        		}
        		disk.write(inode.ptr[12], singlyIndiBlock);       //save change
        		...
        		if (fresh) {
        			if(mode == MODE.r) {
                        return DirectBlock.hole;
                    } else if ((doublyIndiBlock.ptr[ptrNumber5] = freeMap.find()) == 0) {
                        return null;
                    }
        		...
        		return new DirectBlock(disk, triplyIndiBlock.ptr[ptrNumber6], blockOff, fresh);
        	} else {         //should never enter this condition
        		return null;
        	}
     } 
